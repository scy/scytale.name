...
---
Unter Unix ist bekanntlich alles eine Datei. Will ein Programm mit einer Datei arbeiten, bekommt es vom Betriebssystem einen Handle auf diese Datei zurück (beliebige Zahl, also eine "Zugriffs-ID").

---
Vordefinierte Handles: 0 (Standardeingabe), 1 (Standardausgabe), 2 (Standardfehlerausgabe).

---
Diese werden vom Betriebssystem oder der Shell bereits mit den korrekten "Dateien" verbunden, z.B. der Tastatur, dem Bildschirm, einer echten Datei oder einer Pipe ("Röhre"), die zu einem anderen Programm führt.
...
Leitet die Ausgabe in eine Datei, die neu angelegt wird. Falls sie bereits existiert, wird sie überschrieben.

---
--color red
--ulon
--center Hier gilt der Grundsatz: "Unix wurde nicht entwickelt, um seine Benutzer daran zu hindern, dumme Dinge zu tun, denn das würde diese auch davon abhalten, schlaue Dinge zu tun." (Doug Gwyn)
--uloff
--color blue
...
--newpage out-append
--heading Anhängen an eine Datei
---
...
Leitet die Ausgabe in eine Datei und hängt dabei an den bestehenden Inhalt an. Falls die Datei noch nicht existiert, wird sie angelegt.
...
Falls die Standardeingabe für einen Befehl nicht aus einer Pipe kommt, liest er eben von der Tastatur (die ja die "Default-Standardeingabe" ist), und zwar so lange, bis man über die Tastenkombination Strg+D (abgekürzt ^D) sagt, dass der Eingabestrom beendet ist. (Wenn dein Script irgendwie "hängt", ist ein vergessener Eingabedateiname oft der Grund.)
...
--newpage in-redir
--heading Standardeingabe ändern
...
Entfernt doppelt vorkommende Zeilen in der Eingabe, die allerdings bereits vorsortiert sein muss. Hat außerdem noch ein paar lustige Bonusfunktionen, wie immer siehe --help.
...
--newpage count-for
--heading Zählende for-Schleifen
...
--endshelloutput
---
`Backticks` sind äquivalent, aber nicht schachtelbar und schwerer zu lesen, daher wird $() dringend empfohlen.
...
4
--endshelloutput
Die einzelnen Operanden einer Rechenanweisung müssen als einzelne Parameter übergeben werden (also mit Leerzeichen abgetrennt). Von der Shell ausgewertete Zeichen wie * (Multiplikation) müssen natürlich mit \ oder Singlequotes escaped werden.

expr arbeitet nur mit ganzen Zahlen. Für komplexere Berechnungen hilft bc, aber auch bash kann in begrenztem Maße rechnen: $(()).

--newpage wget
--heading HTTP-Zugriffe mit wget und curl

wget ist auf den meisten Systemen verfügbar, während curl praktische Funktionen zum Senden von Daten und Simulieren von Formularaktivitäten besitzt, daher sollte man zumindest mit beiden Tools ansatzweise umgehen können.
...
Merke: "-" ist bei vielen Tools eine Abkürzung für die Standardausgabe. Alternativ kann man meist auch "/dev/stdout" verwenden.
...
Parameter: -r (erweiterte Ausdrücke), -n (nur Matches mit p-Flag ausgeben, nicht matchende Zeilen nicht).
...
Man machte mich nach dem Vortrag darauf aufmerksam, dass es praktisch wäre, die Leute darauf hinzuweisen, dass man Bash-Befehle auch in einer Datei niederschreiben kann, um sie dann alle auf einmal auszuführen; sowas nennt sich dann ein "Script".

Ich hielt das für trivial genug, um es nicht zu erwähnen, aber ja, das kann man. Präferierte Dateinamenserweiterung ist .sh, man kann das Script mit "chmod u+x dateiname.sh" ausführbar machen, wenn als erste Zeile folgendes in der Datei steht:
